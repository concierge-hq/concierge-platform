...
def construct(description: Optional[str] = None):
    ...
    Usage:
        ...
        from concierge.core import construct
    ...
    def decorator(cls: Type[T]) -> Type[T]:
        if not issubclass(cls, BaseModel):
            raise TypeError(
                f"@construct can only be applied to Pydantic BaseModel classes. {cls.__name__} is not a BaseModel."
            )
...
def validate_construct(obj, name: str = "Object"):
    ...
    if not is_construct(obj):
        raise TypeError(f"{name} must be a @construct. Apply @construct decorator to your Pydantic BaseModel.")
