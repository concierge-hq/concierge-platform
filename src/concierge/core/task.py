...
class Task:
    ...
    def to_schema(self) -> dict:
        ...
        return {"name": self.name, "description": self.description, "input_schema": InputModel.model_json_schema()}
...
class task:
    ...
    def __call__(self, func: Callable) -> Callable:
        ...
        if isinstance(func, classmethod):
            raise TypeError(
                "Invalid task definition: @task decorator cannot be applied to @classmethod.\n"
                "Tasks must be regular instance methods.\n"
                "Example:\n"
                "  @stage()\n"
                "  class MyStage:\n"
                "      @task()\n"
                "      def my_task(self, state, ...):\n"
                "          ...\n"
            )

        if isinstance(func, staticmethod):
            raise TypeError(
                "Invalid task definition: @task decorator cannot be applied to @staticmethod.\n"
                "Tasks must be regular instance methods.\n"
                "Example:\n"
                "  @stage()\n"
                "  class MyStage:\n"
