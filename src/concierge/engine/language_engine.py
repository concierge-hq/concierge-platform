...
from concierge.core.results import (
    ErrorResult,
...
class LanguageEngine:
    ...
    def get_initial_message(self) -> str:
        ...
        result = StateUpdateResult(message="Session started successfully.", presentation_type=ComprehensivePresentation)
    ...
    async def process(self, llm_json: dict) -> str:
        ...
        try:
            ...
            if action_type == "handshake":
                ...
                action = MethodCallAction(task_name=llm_json["task"], args=llm_json.get("args", {}))
                ...
                action = StageTransitionAction(target_stage=llm_json["stage"])
                ...
                result = StateUpdateResult(
                    message="State populated successfully.", presentation_type=ComprehensivePresentation
                )
                return self._format_state_update(result)
                ...
                return self._format_error_result(
                    ErrorResult(
                        message=f"Unknown action type: {action_type}", presentation_type=ComprehensivePresentation
                    )
                )
