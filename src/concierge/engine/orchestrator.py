...
from concierge.core.results import ErrorResult, Result, StateInputRequiredResult, TaskResult, TransitionResult
...
from concierge.presentations import ComprehensivePresentation, StateInputPresentation
...
class Orchestrator:
    ...
    async def execute_method_call(self, action: MethodCallAction) -> Result:
        ...
        if result["type"] == "task_result":
            ...
            return TaskResult(
                task_name=action.task_name, result=result["result"], presentation_type=ComprehensivePresentation
            )
        else:
            return ErrorResult(
    ...
    async def execute_stage_transition(self, action: StageTransitionAction) -> Result:
        ...
        validation = self.workflow.validate_transition(stage.name, action.target_stage, global_state, source_state)
        ...
        if propagation_config != "none":
            ...
            if propagation_config == "all":
                ...
                state_to_transfer = {
                    key: value for key, value in source_state_dict.items() if key in propagation_config
                }
            else:
                state_to_transfer = {}

            if state_to_transfer:
                await state_mgr.update_stage_state(self.session_id, action.target_stage, state_to_transfer)

        # Update current stage in state_manager
        await state_mgr.update_current_stage(self.session_id, action.target_stage)

        self.history.append({"action": ACTION_STAGE_TRANSITION, "from": stage.name, "to": action.target_stage})

        return TransitionResult(
            from_stage=stage.name, to_stage=action.target_stage, presentation_type=ComprehensivePresentation
        )
    ...
    async def populate_state(self, state_data: dict) -> None:
        ...
        state_mgr = get_state_manager()
        await state_mgr.update_stage_state(self.session_id, current_stage.name, state_data)
