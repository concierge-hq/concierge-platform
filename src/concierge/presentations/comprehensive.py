...
from concierge.external.contracts import ACTION_METHOD_CALL, ACTION_STAGE_TRANSITION, StageTransition, TaskCall
...
class ComprehensivePresentation(Presentation):
    ...
    def render_json(self, orchestrator) -> dict:
        ...
        return {"content": self.content, "tools": tools}
    ...
    def _format_tasks(self, stage) -> str:
        ...
        for task_name, task in stage.tasks.items():
            ...
            task_lines.append("    Call Format:")

            example_call = TaskCall(
                action=ACTION_METHOD_CALL, task=task_name, args=self._generate_example_args(task_schema["input_schema"])
            )
    ...
    def _format_transitions(self, stage) -> str:
        ...
        for target_stage in stage.transitions:
            ...
            transition_call = StageTransition(action=ACTION_STAGE_TRANSITION, stage=target_stage)
