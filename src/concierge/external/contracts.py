...
class TaskCall(BaseModel):
    ...
    action: Literal["method_call"] = Field(description="Action type identifier for task execution")
    task: str = Field(description="Name of the task to invoke", examples=["search", "add_to_cart", "checkout"])
    args: dict[str, Any] = Field(
        default_factory=dict,
        description="Arguments to pass to the task",
        examples=[{"symbol": "AAPL"}, {"symbol": "GOOGL", "quantity": 10}],
    )
...
class StageTransition(BaseModel):
    ...
    action: Literal["stage_transition"] = Field(description="Action type identifier for stage transition")
    stage: str = Field(description="Target stage name to transition to", examples=["portfolio", "checkout", "browse"])
...
class TerminateSession(BaseModel):
    ...
    action: Literal["terminate_session"] = Field(description="Action type identifier for session termination")
    reason: str | None = Field(
        default=None, description="Optional reason for termination", examples=["completed", "user_request", "timeout"]
    )
...
class StateInput(BaseModel):
    ...
    action: Literal["state_input"] = Field(description="Action type identifier for state input")
    state_updates: dict[str, Any] = Field(
        ...
        examples=[{"username": "john_doe", "age": 25}, {"symbol": "AAPL", "quantity": 10}],
    )
