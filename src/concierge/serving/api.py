...
def initialize_api(
    session_managers: Dict[str, SessionManager], tracker: Optional[Any] = None, state_manager: Optional[Any] = None
):
...
async def get_stats() -> Dict[str, Any]:
    ...
    return {
        "workflows": {"total": workflow_count, "active": workflow_count},
        "executions": {
...
async def get_workflow_details(workflow_name: str) -> Dict[str, Any]:
    ...
    for stage_name, stage in workflow.stages.items():
        tasks = {
            task_name: {"name": task.name, "description": task.description} for task_name, task in stage.tasks.items()
        }
        stages[stage_name] = {
            "name": stage.name,
        ...
        nodes.append(
            {
                ...
                "position": {"x": (index % 3) * 250, "y": (index // 3) * 200},
            }
        )
        for target_stage in stage.transitions:
            edges.append({"id": f"{stage.name}->{target_stage}", "source": stage.name, "target": target_stage})
...
async def get_session_history(session_id: str) -> Dict[str, Any]:
    ...
    if context.tracker is None:
        raise HTTPException(status_code=503, detail="History tracking requires database configuration")
    ...
    return {"session_id": session_id, "step_count": len(steps), "steps": steps}
...
async def get_statistics() -> Dict[str, Any]:
    ...
    active_session_count = sum(len(manager.get_active_sessions()) for manager in context.session_managers.values())
    ...
    return {"total_workflows": len(workflow_list), "active_sessions": active_session_count, "workflows": workflow_stats}
...
async def execute_workflow(http_request: Request):
    ...
    if not workflow_name:
        ...
        if len(managers) == 1:
            ...
            raise HTTPException(status_code=400, detail="workflow_name required when multiple workflows available")
    ...
    if not session_id:
        ...
        if session_id not in session_manager.sessions:
            raise HTTPException(
                status_code=404, detail=f"Session {session_id} not found. Send handshake to create new session."
            )
    ...
    return Response(content=result, media_type="application/json", headers={"X-Session-Id": session_id})
