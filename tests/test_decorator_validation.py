...
from concierge import State, stage, task, workflow
...
def test_double_stage_decorator_fails():
    ...
    with pytest.raises(
        TypeError, match="Cannot use @stage decorator on a Stage object that has already been decorated"
    ):
...
def test_multiple_stages_in_workflow():
    ...
    class MultiStageWorkflow:
        ...
        transitions = {Stage1: [Stage2], Stage2: [Stage1]}
...
def test_double_workflow_decorator_fails():
    ...
    with pytest.raises(
        TypeError, match="Cannot use @workflow decorator on a Workflow object that has already been decorated"
    ):
