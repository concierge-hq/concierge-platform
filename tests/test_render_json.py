...
from concierge.presentations import BriefPresentation, ComprehensivePresentation
...
class TestShoppingWorkflow:
    ...
    transitions = {shopping: [checkout], checkout: []}
...
class TestRenderJson:
    ...
    def test_comprehensive_presentation_render_json(self):
        ...
        engine = LanguageEngine(workflow=TestShoppingWorkflow._workflow, session_id="test-123", output_format="json")

        task_result = TaskResult(
            task_name="add_to_cart", result={"success": True}, presentation_type=ComprehensivePresentation
        )
            ...
            "tools": [
                ...
                },
                {
                    ...
                    "input_schema": {
                        "type": "object",
                        "properties": {
                            "reason": {"type": "string", "description": "Optional reason for ending the session"}
                        },
                        "required": [],
                    },
                },
    ...
    def test_brief_presentation_render_json(self):
        ...
        engine = LanguageEngine(workflow=TestShoppingWorkflow._workflow, session_id="test-456", output_format="json")

        task_result = TaskResult(
            task_name="search_products", result={"results": ["item1", "item2"]}, presentation_type=BriefPresentation
        )
        content = TaskResultMessage().render(task_result)
    ...
    def test_tool_schema_format(self):
        ...
        engine = LanguageEngine(workflow=TestShoppingWorkflow._workflow, session_id="test-789", output_format="json")

        task_result = TaskResult(
            task_name="add_to_cart", result={"cart_size": 3}, presentation_type=ComprehensivePresentation
        )
        content = TaskResultMessage().render(task_result)
    ...
    async def test_language_engine_output_format_text(self):
        ...
        engine = LanguageEngine(workflow=TestShoppingWorkflow._workflow, session_id="test-text", output_format="text")

        result = await engine.process(
            {"action": "method_call", "task": "add_to_cart", "args": {"product_id": "123", "quantity": 2}}
        )
    ...
    async def test_language_engine_output_format_json(self):
        ...
        engine = LanguageEngine(workflow=TestShoppingWorkflow._workflow, session_id="test-json", output_format="json")
    ...
    async def test_language_engine_default_format(self):
        ...
        engine = LanguageEngine(workflow=TestShoppingWorkflow._workflow, session_id="test-default")
    ...
    async def test_terminate_session(self):
        ...
        result = await engine.process({"action": "terminate_session", "reason": "completed"})
