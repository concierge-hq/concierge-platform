...
class TestStateInputPresentation:
    ...
    def test_state_input_presentation_render_json(self):
        ...
        engine = LanguageEngine(
            workflow=TestStateInputWorkflow._workflow, session_id="test-state-input", output_format="json"
        )
            ...
            "tools": [
                {
                    ...
                    "input_schema": {
                        "type": "object",
                        "properties": {
                            "user_email": {"type": "string", "description": "Value for user_email"},
                            "payment_method": {"type": "string", "description": "Value for payment_method"},
                        },
    ...
    def test_state_input_with_empty_fields(self):
        ...
        engine = LanguageEngine(
            workflow=TestStateInputWorkflow._workflow, session_id="test-empty", output_format="json"
        )
    ...
    async def test_state_cleared_after_providing(self):
        ...
        engine = LanguageEngine(
            workflow=TestStateInputWorkflow._workflow, session_id="test-clear", output_format="json"
        )

        engine.orchestrator.required_state_fields = ["user_email", "payment_method"]

        await engine.process(
            {"action": "state_input", "state_updates": {"user_email": "test@example.com", "payment_method": "visa"}}
        )
