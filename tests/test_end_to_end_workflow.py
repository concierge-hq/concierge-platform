...
class ConfirmationStage:
    ...
    def get_order_details(self, state: State) -> dict:
        """Get order details"""
        return {"order_id": state.get("order_id"), "email": state.get("email"), "status": "confirmed"}

    @task(description="Send confirmation email")
    def send_confirmation_email(self, state: State) -> dict:
        """Send email"""
        return {"success": True, "sent_to": state.get("email")}
...
class EcommerceWorkflow:
    ...
    transitions = {browse: [checkout], checkout: [confirmation], confirmation: []}
...
async def test_end_to_end_ecommerce_flow():
    ...
    engine = LanguageEngine(workflow=EcommerceWorkflow._workflow, session_id=session_id, output_format="json")
    ...
    result = await engine.process({"action": "method_call", "task": "search_products", "args": {"query": "laptop"}})
    result_json = json.loads(result)
    assert "content" in result_json
    assert "tools" in result_json
    ...
    result = await engine.process(
        {"action": "method_call", "task": "add_to_cart", "args": {"product_id": "prod1", "quantity": 1}}
    )
    result_json = json.loads(result)
    assert "success" in result_json["content"].lower() or "cart_size" in result_json["content"]

    result = await engine.process(
        {"action": "method_call", "task": "add_to_cart", "args": {"product_id": "prod2", "quantity": 2}}
    )
    result_json = json.loads(result)
    assert "tools" in result_json

    # Step 4: View cart
    result = await engine.process({"action": "method_call", "task": "view_cart", "args": {}})
    result_json = json.loads(result)
    assert "content" in result_json

    # Step 5: Try to transition to checkout WITHOUT providing state (should fail)
    result = await engine.process({"action": "stage_transition", "stage": "checkout"})
    result_json = json.loads(result)
    ...
    result = await engine.process({"action": "method_call", "task": "calculate_total", "args": {}})
    result_json = json.loads(result)
    assert "content" in result_json
    assert "total" in result_json["content"].lower()

    # Step 9: Complete purchase
    result = await engine.process({"action": "method_call", "task": "complete_purchase", "args": {}})
    result_json = json.loads(result)
    assert "success" in result_json["content"].lower() or "order" in result_json["content"].lower()

    # Step 10: Transition to confirmation
    result = await engine.process({"action": "stage_transition", "stage": "confirmation"})
    result_json = json.loads(result)
    ...
    result = await engine.process({"action": "method_call", "task": "get_order_details", "args": {}})
    result_json = json.loads(result)
    assert "content" in result_json
    assert "order" in result_json["content"].lower() or "confirmed" in result_json["content"].lower()

    # Step 12: Send confirmation email
    result = await engine.process({"action": "method_call", "task": "send_confirmation_email", "args": {}})
    result_json = json.loads(result)
    assert "success" in result_json["content"].lower() or "sent" in result_json["content"].lower()

    # Step 13: Terminate session
    result = await engine.process({"action": "terminate_session", "reason": "order completed"})
    result_json = json.loads(result)
...
async def test_invalid_task_call():
    ...
    result = await engine.process({"action": "method_call", "task": "nonexistent_task", "args": {}})
    result_json = json.loads(result)
...
async def test_missing_task_args():
    ...
    result = await engine.process({"action": "method_call", "task": "search_products", "args": {}})
    result_json = json.loads(result)
...
async def test_state_persistence_across_stages():
    ...
    await engine.process(
        {"action": "method_call", "task": "add_to_cart", "args": {"product_id": "test_product", "quantity": 5}}
    )
    ...
    await engine.process({"action": "stage_transition", "stage": "checkout"})
