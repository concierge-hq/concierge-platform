...
class StockWorkflow:
    ...
    transitions = {browse: [portfolio], portfolio: [browse]}
...
def test_stock_workflow_search():
    ...
    async def run():
        ...
        response = await engine.process({"action": "method_call", "task": "search", "args": {"symbol": "AAPL"}})
...
def test_stock_workflow_add_to_cart():
    ...
    async def run():
        ...
        response = await engine.process(
            {"action": "method_call", "task": "add_to_cart", "args": {"symbol": "GOOGL", "quantity": 5}}
        )
...
def test_stock_workflow_transition():
    ...
    async def run():
        ...
        response = await engine.process({"action": "stage_transition", "stage": "portfolio"})
...
def test_stock_workflow_full_conversation():
    ...
    async def run():
        ...
        response1 = await engine.process({"action": "method_call", "task": "search", "args": {"symbol": "AAPL"}})
        assert "Task 'search' executed successfully." in response1
        assert "Found AAPL: $150.00" in response1
        assert orch.get_current_stage().name == "browse"

        response2 = await engine.process(
            {"action": "method_call", "task": "add_to_cart", "args": {"symbol": "AAPL", "quantity": 10}}
        )
        ...
        response3 = await engine.process({"action": "stage_transition", "stage": "portfolio"})
        assert "Successfully transitioned from 'browse' to 'portfolio'." in response3
        assert orch.get_current_stage().name == "portfolio"

        response4 = await engine.process({"action": "method_call", "task": "view_holdings", "args": {}})
...
def test_stock_workflow_invalid_action():
    ...
    async def run():
        ...
        response = await engine.process({"action": "invalid_action", "data": "whatever"})
...
def test_stock_workflow_invalid_task():
    ...
    async def run():
        ...
        response = await engine.process({"action": "method_call", "task": "nonexistent_task", "args": {}})
...
def test_stock_workflow_invalid_transition():
    ...
    async def run():
        ...
        response = await engine.process({"action": "stage_transition", "stage": "nonexistent_stage"})
