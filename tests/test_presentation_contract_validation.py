...
from concierge.external.contracts import StageTransition, TaskCall
...
def extract_json_examples(presentation_output: str) -> dict[str, list[dict]]:
    ...
    examples = {"task_calls": [], "transitions": []}
...
def test_presentation_task_call_examples_match_contract():
    ...
    async def run():
        ...
        response = await engine.process(
            {"action": "method_call", "task": "test_task", "args": {"arg1": "test", "arg2": 42}}
        )
    ...
    async def run():
        ...
        for i, task_example in enumerate(examples["task_calls"]):
            try:
                ...
                assert "Unknown action type" not in result, f"Task call example {i} was rejected by language engine!"
